...
from sympy import symbols, log, exp, limit, KroneckerDelta, diff, \
    Product, factor, Pow, Symbol, simplify, Limit, Mul, expand
...
for i, state in enumerate(oploop):
    ...
    def __rec_lims(term):
        ...
        def __apply(term):
            ...
            return ttype(*[ttype(*[__rec_lims(h) for h in has]),
                           __rec_lims(ttype(*hasnt))])

        save = term
        term = expand(term)
        if save != term:
            return __rec_lims(__apply(term))
        lim = limit(term, Xi, 0)
        print(term, lim)
        return lim
