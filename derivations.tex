...
\begin{align}
 \lhs{dnkdotdT}{\mathcal{J}_{k + 2, 1}} &\pluseq  \rhs{dnkdotdT}{V \left(\nu_{k,i} R_{i} \frac{\partial c }{\partial T }_{i} + \nu_{k,i} \frac{\partial R }{\partial T }_{i} c_{i}\right)} & \mathcond{dnkdotdT}{k = 1, \ldots, \ns}
\end{align}
the rate of progress temperature derivative for all reaction types---excluding Chebyshev and P-Log---is:
\loadeq{dropdT}
while for P-Log reactions:
\loadeq{dropplogdT}
and for Chebyshev reactions:
\loadeq{dropchebdT}
