...
\newcommand{\conp}{CONP}
\newcommand{\conv}{CONV}
\newcommand{\dconp}{\ensuremath{\text{for \conp,}}}
\newcommand{\dconv}{\ensuremath{\text{for \conv,}}}
...
\Crefrange{e:dpr_mix_dt}{e:dpr_mix_de} may be recast in a more generalized form using temporary variables $\Theta_{\partial P_{r, i}, \ldots}$ and $\bar{\theta}_{\partial P_{r, i}, \ldots}$ to replace terms in the derivative:
\begin{align}
...
with:
\begin{subequations}
\label{e:dpr_theta_mix}
\begin{align}
 \Theta_{P_{r,i}, \partial T} &= \frac{1}{T} \left(\beta_{0} - \beta_{\infty} + \frac{E_{a, 0}}{T \Ru} - \frac{E_{a, \infty}}{T \Ru}\right) \label{e:dtheta_pr_dt} \\
 \bar{\theta}_{P_{r, i}, \partial T} &= - \frac{[C] k_{0, i} \alpha_{\ns,i}}{T k_{\infty, i}} \\
 \bar{\theta}_{P_{r, i}, \partial n_j} &= - \alpha_{\ns,i} + \alpha_{j,i} \\
 \Theta_{P_{r,i}, \partial V} &= - \frac{1}{V} \\
 \bar{\theta}_{P_{r, i}, \partial V} &= \frac{[C] k_{0, i} \alpha_{\ns,i}}{V k_{\infty, i}} \\
 \bar{\theta}_{P_{r, i}, \partial P} &= \frac{k_{0, i} \alpha_{\ns,i}}{T k_{\infty, i} \Ru}
\end{align}
\end{subequations}

For a third-body efficiency with all $\alpha_{k,i} = 1$, the reduced pressures derivatives may be expressed as~\cref{e:dpr_de}, with:
\begin{subequations}
 \label{e:dpr_theta_unity}
 \begin{align}
  \Theta_{P_{r,i}, \partial T} &= \frac{1}{T} \left(\beta_{0} - \beta_{\infty} - 1 + \frac{E_{a, 0}}{T \Ru} - \frac{E_{a, \infty}}{T \Ru}\right) \\
  \bar{\theta}_{P_{r, i}, \partial T} &= 0 \\
  \bar{\theta}_{P_{r, i}, \partial n_j} &= 0 \\
  \Theta_{P_{r,i}, \partial V} &= 0 \\
  \bar{\theta}_{P_{r, i}, \partial V} &= 0 \\
  \bar{\theta}_{P_{r, i}, \partial P} &= \frac{k_{0, i}}{k_{\infty, i}} \frac{1}{\Ru T}
 \end{align}
\end{subequations}

If species $m$ serves as the third-body concentration, the temporary terms are:
\begin{subequations}
 \label{e:dpr_theta_spec}
 \begin{align}
  \Theta_{P_{r,i}, \partial T} &= \frac{1}{T} \left(\beta_{0} - \beta_{\infty} + \frac{E_{a, 0}}{T \Ru} - \frac{E_{a, \infty}}{T \Ru}\right) \\
  \bar{\theta}_{P_{r, i}, \partial T} &= - \frac{[C] k_{0, i} \delta_{\ns m}}{T k_{\infty, i}} \\
  \bar{\theta}_{P_{r, i}, \partial n_j} &= - \delta_{\ns m} + \delta_{j m} \\
  \Theta_{P_{r,i}, \partial V} &= - \frac{1}{V} \\
  \bar{\theta}_{P_{r, i}, \partial V} &= \frac{[C] k_{0, i} \delta_{\ns m}}{V k_{\infty, i}} \\
  \bar{\theta}_{P_{r, i}, \partial P} &= \frac{k_{0, i} \delta_{\ns m}}{T k_{\infty, i} \Ru}
 \end{align}
\end{subequations}
