...
\begin{dcases}
\rhs{dfi_dt}{0}[c] & \cond{dfi_dt}{if Lindemann}[l] \\
\rhs{dfi_dt}{ - \frac{B_{Troe}}{F_{cent} P_{r, i} \left(A_{Troe}^{2} + B_{Troe}^{2}\right)^{2} \log{\left (10 \right )}}
	     \Biggl(} & \cond{dfi_dt}{} \nonumber \\
\rhs{dfi_dt}{\mathindent 2 A_{Troe} F_{cent} \left(0.14 A_{Troe} + B_{Troe}\right)
	     \left(P_{r, i} \Theta_{P_{r,i}, \partial T} + \bar{\theta}_{P_{r, i}, \partial T}\right)
	     \log{\left (F_{cent} \right )} + } & \cond{dfi_dt}{} \nonumber \\
\rhs{dfi_dt}{\mathindent P_{r, i} \frac{\text{d} F_{cent} }{\text{d} T }
	      \biggl( 2 A_{Troe} \left(1.1762 A_{Troe} - 0.67 B_{Troe}\right) \log{\left (F_{cent} \right )} - } & \cond{dfi_dt}{} \nonumber \\
\rhs{dfi_dt}{\mathindent[plussomeextrastu] B_{Troe} \left(A_{Troe}^{2} + B_{Troe}^{2}\right) \log{\left (10 \right )}\biggr)\Biggr)} & \cond{dfi_dt}{if Troe}[l] \\
\rhs{dfi_dt}{- \frac{X \left(\frac{\operatorname{exp}\left({- \frac{T}{c}}\right)}{c} - \frac{a b}{T^{2}} \operatorname{exp}\left({- \frac{b}{T}}\right)\right)}{a \operatorname{exp}\left({- \frac{b}{T}}\right) + \operatorname{exp}\left({- \frac{T}{c}}\right)} + \frac{e}{T} - \frac{2 X^{2} \log{\left (a \operatorname{exp}\left({- \frac{b}{T}}\right) + \operatorname{exp}\left({- \frac{T}{c}}\right) \right )}}{P_{r, i} \log^{2}{\left (10 \right )}} \left(P_{r, i} \Theta_{P_{r,i}, \partial T} + \bar{\theta}_{P_{r, i}, \partial T}\right) \log{\left (P_{r, i} \right )}} & \cond{dfi_dt}{if SRI}[l] \\
\end{dcases}
\end{align}
