...
Similarly,~\cref{e:dnkdotdT_compact} is substituted into~\cref{e:dEdotdT_nojac}, giving:
\loadeq{dEdotdT}
...
\loadeq{dnkdotdT}
As \texttt{pyJac} evaluates these derivative entries by looping over the reactions $i$ in the model, we convert~\cref{e:dnkdotdT} to an update form:
\begin{align}
 \lhs{dnkdotdT}{\mathcal{J}_{k + 2, 1}} &\pluseq  \rhs{dnkdotdT}{V \left(\nu_{k,i} R_{i} \frac{\partial c_{i}}{\partial T } + \nu_{k,i} \frac{\partial R_{i}}{\partial T } c_{i}\right)} & \mathcond{dnkdotdT}{k = 1, \ldots, \ns - 1}
\end{align}
...
\subsubsection{Molar source term derivatives}
To obtain a final form for the molar source term derivative with respect to the state paramter,~\cref{e:dnkdotdE_compact,e:dwdot_de} are combined:
\loadeq{dnkdotdE}

The derivative of the net rate of progress with respect to the state paramter is given by~\cref{e:dropi_de} for arrhenius-based reactions,~\cref{e:dropi_plog_de,e:dropi_cheb_de} for P-Log and Chebyshev reactions for a \conv~system, and~\cref{e:dropi_de} for a \conp~system.
...
\loadeq{dnkdotdnj}
As in~\cref{s:dtdotdnj}, this is converted to an form that may be updated per-reaction $i$:
\begin{align}
