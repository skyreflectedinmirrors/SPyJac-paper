...
\newcommand{\avx}{\texttt{avx2}}
\newcommand{\sse}{\texttt{sse4.2}}
\newcommand{\gpuold}{\texttt{C2075}}
\newcommand{\gpunew}{\texttt{K40m}}
...
\Cref{f:source} explores the performance of the source term evaluations generated by \texttt{pyJac} on the CPU test platforms listed in~\cref{t:cpus}.
...
The reasons for this lack of vectorization are quite technical and somewhat outside the scope of this work, however briefly speaking it is possible vectorization might be achieved on POCL via combination of relaxed math (``-cl-fast-relaxed-math''), use of explicit OpenCL vector types and future improvements to LLVM's vectorization module.
...
\begin{figure}[htb]
  ...
  \begin{subfigure}{0.45\linewidth}
      ...
      \caption{The mean runtime per condition for each chemical model using the Intel OpenCL and OpenMP on both CPUs studied; the names in the legend correspond to the identifiers listed in~\cref{t:cpus}}
      \label{F:intel_source_noorm}
  \end{subfigure}
  \begin{subfigure}{0.45\linewidth}
      \includegraphics[width=\textwidth]{intel_source.pdf}
      \caption{The speedup achieved over the baseline OpenMP parallelization by both the unvectorized and wide-vectorized Intel OpenCL codes for each chemical model.}
      \label{F:intel_source}
  \end{subfigure}
  \\
  \begin{subfigure}{0.45\linewidth}
      \includegraphics[width=\textwidth]{pocl_source_nonorm.pdf}
      \caption{The mean runtime per condition of the Portable OpenCL runtime compared to OpenMP parallelization for each chemical model.}
      \label{F:pocl_source}
  \end{subfigure}
 \label{f:source}
\end{figure}
