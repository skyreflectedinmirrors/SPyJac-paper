...
Finally,~\cref{F:vector_data} shows a vectorized data-ordering that negates the disadvantages of a shallow, ``C''-ordered SIMD vectorization on the CPU (\cref{F:row_major_split}) and a deep, ``F''-ordered SIMT vectorization on the GPU (\cref{F:column_major_split}).
...
The current code evaluates the following thermo-chemical source terms:
