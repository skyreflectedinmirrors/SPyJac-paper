...
Indeed, the shallow-vectorized sparse OpenCL code is faster than the dense OpenMP code in all cases.
\Cref{F:sparse_vs_dense_speedup} shows the speedup of the sparse\slash dense shallow-vectorized OpenCL the over the same sparse\slash dense Jacobian format on OpenMP; a speedup of \SIrange{3.7}{4.2}{$\times$} is exhibited for the dense OpenCL code, increasing to \SIrange{6.6}{8.4}{$\times$} for the sparse Jacobian.
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{Performance of sparse and dense Jacobian evaluation in \texttt{pyJac}.}
