...
In contrast, the OpenMP code requires \textasciitilde\num{4} cores to match the performance of the shallow-vectorized OpenCL code in this case, hence parallel overhead concerns become increasingly important for 8--16 cores.
Indeed, as will be investigated in~\cref{S:jacobian_results} the strong scalings for sparse Jacobian evaluation---the most computationally intensive task in this work for which the efficiency will investigated---are far more similar between Intel OpenCL and OpenMP.
...
\Cref{F:source_simd_scaling} shows the SIMD efficiency of source term evaluation in \texttt{pyJac} on a single core of the \avx/ machine; the larger models have higher SIMD-efficiency of \textasciitilde\num{0.75}, and the smaller models have lower SIMD-efficiency, \textasciitilde\num{0.625}.
...
\Cref{F:sparse_vs_dense_speedup} shows the speedup of the sparse\slash dense shallow-vectorized OpenCL the over the same sparse\slash dense Jacobian format on OpenMP; the dense OpenCL is \SIrange{3.7}{4.2}{$\times$} faster than the corresponding dense OpenMP code, this speedup increases to \SIrange{6.6}{8.4}{$\times$} for the sparse Jacobian.
...
Once again, the sparse OpenCL code is far less negatively impacted by the indirect lookup, and is only \SIrange{1.8}{3.2}{$\times$} slower than its dense counterpart, while the sparse OpenMP code is significantly (\SIrange{2.25}{6.2}{$\times$}) slower than the dense version.
In~\cref{F:sparse_vs_dense_sse_speedup}, the speedup of the sparse and dense shallow-vectorized OpenCL codes are compared to their OpenMP version; the dense OpenCL code is \SIrange{3.8}{4.2}{$\times$} faster, while the sparse shallow-vectorization achieves a speedup of \SIrange{6.6}{9.1}{$\times$}.
...
In~\cref{F:sparse_jac_scaling}, the parallel scaling efficiency of the sparse shallow-vectorized OpenCL is compared to the sparse OpenMP code.
...
In contrast,~\cref{F:dense_jac_scaling} shows scaling for the shallow-vectorized dense Jacobian OpenCL code that is higher than for source-rate evaluation, but not as good as for sparse Jacobian evaluation.
In this case, the USC-Mech II model\todo{note: don't have data for ipentanol yet} has the worst scaling for both codes; ranging from \numrange{0.96}{0.63} and \numrange{0.91}{0.34} for OpenMP and OpenCL respectively.
