...
Finally, unless otherwise stated the \conp/ assumption was used (the differences between \conp/ and \conv/ evaluations will be explored) and the runtimes reported are for the number of conditions specified in~\cref{T:error}.
...
\todo{rerun NVIDIA on srv2 -- see if that high variability in isopentanol persists}
\begin{figure}[htb]
   \centering
  \begin{subfigure}[t]{0.48\linewidth}
...
In general, the ``C''-ordered wide-vectorization is \textasciitilde\SI{1.05}{1.2}{$\times$} faster on the \newgpu/ GPU while the ``C'' and ``F''-ordered data are nearly equivalent for the \gpuold/ GPU, except for the isopentanol model where the ``F''-ordered data is \textasciitilde\SI{1.4}{$\times$} faster (though, we note that there was high variablity in this case). \todo{again, pending rerun on \gpuold/}
