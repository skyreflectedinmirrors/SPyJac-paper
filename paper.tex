...
Niemeyer et al.~\cite{Niemeyer:2011aa} implemented an explicit solver for non-stiff chemistry on a\revisedelete{ Tesla C2075} GPU, achieving a speedup of nearly two orders of magnitude over a sequential \revise{CPU} code\revisedelete{on an Intel Xeon X5650 CPU}.
...
\revisedelete{It is important to note that while }
\revise{While OpenCL provides a simple interface to enable cross-platform execution, and significant speedups were achieved via shallow-vectorized OpenCL code in this work, there are some serious potential pitfalls in its use.
...
\revise{To demonstrate the type of issue discussed, we created a minimum working example~\cite{Nvidia_mwe} that shows a failure of Nvidia's OpenCL runtime corresponding to the GPU driver version \num{375.26} on a Tesla \gpunew/ GPU; simply changing to another runtime (e.g., Intel) produces the correct result---with no code changes or recompilation.
...
\revise{The new molar-based formulation for \texttt{pyJac} resulted in highly sparse chemical kinetic Jacobians,
