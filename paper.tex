...
In this article we
\begin{itemize}
 \item Derive and \add{reproducibly} verify a new Jacobian formulation that greatly increases sparsity\delete{: \texttt{pyJac v2}};
 \item \delete{Enable}\add{Detail the implementation of} cross-platform SIMD\slash SIMT vectorization for CPUs, GPUs, and other accelerators;
 \item \add{Investigate the performance of SIMD\slash SIMT-vectorization for a wide of range chemical kinetic models, and compare with the previous version of our analytical chemical kinetic Jacobian code~\cite{Niemeyer:2016aa}; and finally}
...
\add{These efforts are made publicly available (see~\cref{A:availability}) via the open-source, high-performance, chemical kinetics code \texttt{pyJac}}.
...
In addition to parallel OpenMP evaluation on the CPU, \add{this work}\delete{the new version of \texttt{pyJac}} enabled the shallow-vectorized evaluation of the chemical-kinetic source terms and analytical Jacobian on both the CPU and GPU via OpenCL.
