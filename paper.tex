...
A code generation platform, \texttt{pyJac}, for single-instruction, multiple-data (SIMD) and single-instruction, multiple thread (SIMT) vectorized constant-pressure\slash constant-volume thermo-chemical source-term and sparse\slash dense chemical kinetic Jacobian evaluation was developed and validated for a wide range of chemical kinetic models.
Speedups of \SIrange{3.40}{4.08}{$\times$} were found for shallow-vectorized OpenCL source-rate evaluation compared a parallel OpenMP code on an \avx/ CPU, increasing to \SIrange{6.63}{9.44}{$\times$} and \SIrange{3.03}{4.23}{$\times$} for sparse and dense chemical kinetic Jacobian evaluation; in addition, the performance of a \gpunew/ GPU was compared to a \gpuold/ finding speedups of \SIrange{1.40}{1.88}{$\times$}, \SIrange{1.10}{1.59}{$\times$}, and \SIrange{1.36}{3.0}{$\times$} for source-rate, sparse Jacobian and dense Jacobian evaluation, respectively.
Additionally, the effect of data-ordering was investigated and an ordering specifically formulated for vectorized evaluation proposed; as well, the effect of the \conp/\slash\conv/ assumptions and varying vector widths were studied on source-term evaluation performance.
...
Vectorized data-orderings specifically formulated for shallow\slash deep SIMD\slash SIMT vectorizations were proposed, resulting in a shallow-vectorized ``C'' (row major) speedup of \SIrange{2.14}{2.58}{$\times$} over a ``F'' (column major) ordered shallow-vectorization, compared to a speedup of \SIrange{1.35}{2.09}{$\times$} for the ``C''-ordered OpenMP code (over an ``F''-ordered equivalent).
