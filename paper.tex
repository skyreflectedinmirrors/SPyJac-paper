...
In~\cref{F:gpu_source_scaling}, the number of initial conditions over which the source terms are evaluated on the \gpunew/ GPU is varied in order to determine the effect on the mean run time per condition; the mean run time continues to decrease until \textasciitilde\num{e4} conditions for all chemical models (except the \ce{H2}\slash\ce{CO} model, which is closer to \textasciitilde\num{3e4} conditions), at which point the GPU becomes saturated and performance levels off.
Interestingly, the last data-point for the GRI-Mech 3.0 shows an increase in run time per condition, this is likely due to multiple data transfers to\slash from the GPU required to complete the source-rates in this case; similar (if smaller) increases in run time per condition can be seen for the \ce{iC5H11OH} model near \num{e4} and \num{e5} conditions.\todo{rerunning}
...
In~\cref{F:source_conpvsconv}, we see that choice of the constant-pressure or constant-volume formulation has little to no effect on run time for OpenMP as well as the shallow-vectorized\slash unvectorized Intel OpenCL codes on the \avx/ machine.
...
As in the species evaluation case, the mean run time per-condition drops steadily, for both cases becoming constant after just \num{3e3} states (except for \ce{H2}\slash\ce{CO}, which levels off near \num{e4} conditions).
