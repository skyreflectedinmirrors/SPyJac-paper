...
As expected, the \avx/ machine is moderately faster than the \sse/ CPU; additionally, the wide-vectorized OpenCL code is significantly faster than either the OpenMP or unvectorized OpenCL codes on both processors.

In~\cref{F:intel_source}, the extent of this speedup is detailed; we note that the speedup displayed is calculated per-machine, such that the speedup of the wide-vectorized OpenCL code can be readily assessed.
...
Additionally, the wide-vectorized OpenCL code is \textasciitilde\SIrange{2.5}{3}{$\times$} and \textasciitilde\SIrange{3.5}{4}{$\times$} faster than the OpenMP code for the \sse/ and \avx/ machines respectively.
In contrast,~\cref{F:pocl_source} shows the runtime of deep, wide and unvectorized OpenCL codes on the POCL runtime, as compared to OpenMP parallelization.
...
In general, the ``C''-ordered wide-vectorization is \textasciitilde\SIrange{1.05}{1.2}{$\times$} faster on the \gpunew/ GPU while the ``C'' and ``F''-ordered data are nearly equivalent for the \gpuold/ GPU, except for the isopentanol model where the ``F''-ordered data is \textasciitilde\SI{1.4}{$\times$} faster (though, we note that there was high variablity in this case). \todo{again, pending rerun on \gpuold/}
The ``C''-ordered speedup on the \gpunew/ GPU is counter to what one might expect, as typically speaking coalesced memory access in a wide-vectorization is easier to achieve if ``F''-ordering is used (see~\cref{S:data}).
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{0.48\linewidth}
      ...
      \caption{The effect of vector-width on ``C''-ordered wide-vectorized Intel OpenCL source term evaluations in the \avx/ CPU.}
      \label{F:source_vector_width}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.48\linewidth}
      \includegraphics[width=\textwidth]{source_gpu_cvsf.pdf}
      \caption{The affect on performance of ``C'' vs ``F''-ordering for the wide-vectorized NVIDIA OpenCL code on both GPUs with a vector width of 128.  The speedup presented is calculated per-GPU to better assess the effect of the data-ordering on each GPU.}
...
\Cref{F:source_parallel_scaling} shows the strong parallel scaling efficiency of source rate evaluation in \texttt{pyJac} on the \avx/ machine for both the wide-vectorized Intel OpenCL and OpenMP codes.
...
By the time 4--8 CPU cores are utilized, the runtime of wide-vectorized OpenCL code can be measured in hundreds of milliseconds, and parallel overhead begins to dominate, decreasing efficiency.
In contrast, the OpenMP code requires \textasciitilde\num{4} cores to run as fast as the wide-vectorized OpenCL code in this case, hence parallel overhead concerns become increasingly important for 8--16 cores.
...
\begin{equation}
 ...
 \varepsilon_{\text{SIMD}} = \frac{\bar{t}_{\text{unvec}}}{W \bar{t}_{\text{wide}}}
\end{equation}
where $\bar{t}_{\text{unvec}}$ is the mean runtime per condition of the unvectorized OpenCL code, $\bar{t}_{\text{wide}}$ the same for the wide-vectorized OpenCL code, and $W$, the vector width reported in number of double operations in~\cref{t:cpus}
This measure compares the actual speedup due to wide-vectorization compared to the nominal vector-width of the machine in question.
