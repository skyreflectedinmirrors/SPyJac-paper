...
In~\cref{F:intel_source}, the extent of this speedup is detailed; we note that the speedup displayed is normalized per-machine, such that the speedup of the wide-vectorized OpenCL code can be readily assessed.
...
\begin{figure}[htb]
  ...
  \\
  \begin{subfigure}[t]{0.48\linewidth}
      \includegraphics[width=\textwidth]{source_gpu_conpvsconv.pdf}
      \caption{The affect on performance of``C'' vs ``F''-ordering for the wide-vectorized NVIDIA OpenCL code on both GPUs.  The speedup presented is normalized per-GPU to better assess the effect of the data-ordering on each GPU.}
      \label{F:source_conpvsconv}
  \end{subfigure}
