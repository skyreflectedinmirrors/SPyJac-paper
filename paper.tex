...
\newcommand{\delete}[1]{\sloppy\textcolor{red}{\sout{#1}}} %delete with no strikeout
...
Le et al.~\cite{Le2013596} found a \SIrange{30}{50}{$\times$} speedup for a GPU-based shock-capturing reactive-flow code on a Tesla C2070 as compared to a sequential version of the same on an Intel Xeon X5650 CPU.
...
Niemeyer and Sung~\cite{Niemeyer:2014aa} developed a GPU-based stabilized explicit integrator for use with moderately-stiff chemical kinetics, achieving an order of magnitude speedup on a Tesla C2075 GPU over a multithreaded VODE solver on a six-core Intel X5650 CPU.
...
\end{itemize}
To our knowledge, this is the first open-source, validated effort that enables vectorized chemical-kinetic source-term and Jacobian evaluation for any chemical model on a wide-selection of platforms, i.e., the CPU, GPU and other accelerators.
