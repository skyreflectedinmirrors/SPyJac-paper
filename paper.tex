...
Speedups over a first-order finite-difference \revise{Jacobian} approach \revise{reached up to} \SI{17.60}{$\times$} and \SI{45.13}{$\times$} for dense\slash sparse evaluation \revise{respectively on the GPU, and up to} \SI{55.11}{$\times$} and \SI{245.63}{$\times$} on the CPU.
...
Traditional multi-core parallelism \revise{has} often \revise{been} used to increase central processing unit (CPU) performance, \revise{even as the exponential growth in processing power}---colloquially known as Moore's law---has slowed~\cite{khan2018science}.
...
Yonkee and Sutherland~\cite{Yonkee2016} implemented \delete{GPU-}accelerated evaluations of thermodynamic parameters, multicomponent transport properties, and species production rates for solving partial differential equations (PDEs) \delete{on an Intel Xeon E5-2680} \add{both the CPU} and \delete{a Tesla K20} GPU, \revise{achieving speedups over serial evaluation of thermochemical properties between \SIrange{8}{13}{$\times$} on a 16 core CPU, and \SIrange{20}{40}{$\times$} on the GPU}.
\revise{In addition, \textasciitilde\SI{9}{$\times$} and \textasciitilde\SI{25}{$\times$} speedups} were achieved for the simulation of a partially premixed methanol flame PDE on 16 CPU cores and the GPU, respectively.
