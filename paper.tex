...
The OpenMP Jacobian\slash source rate evaluation kernels, as well as the OpenMP\slash OpenCL wrapping code---responsible for initializing\slash transferring memory, reading input, etc.---was compiled with \texttt{gcc v5.4.0} on the \avx/\slash\gpunew/ platforms, and \texttt{gcc v4.8.5} on the \sse/\slash\gpuold/ machines.
...
Interestingly, the last data-point for the GRI-Mech 3.0 shows an increase in runtime per condition, this is likely due to multiple data transfers to\slash from the GPU required to complete the source rate evaluations in this case; similar (if smaller) increases in runtime per condition can be seen for the \ce{iC5H11OH} model near \num{e4} and \num{e5} conditions.\todo{rerunning}
...
This is again likely due to relatively small amounts of work associated with source rate evaluation; the longest runtime (the isopentanol model on a single core) takes only \textasciitilde\SI{22}{\second} for OpenCL, and \textasciitilde\SI{90}{\second} for OpenMP.
By the time \numrange{4}{8} CPU cores are utilized in the shallow-vectorized OpenCL code, the runtime is just \textasciitilde\SIrange{5}{8}{\sec} and parallel overhead begins to dominate, decreasing efficiency.
In contrast, the OpenMP code running on \num{4} cores is as fast (\ce{H2}\slash\ce{CO}) or slightly slower than the shallow-vectorized OpenCL code on a single core, hence parallel overhead concerns become increasingly important for \numrange{8}{16} OpenMP cores; correspondingly, the OpenMP code's parallel efficiency drops to \num{0.64} for \num{16} cores, similar to the OpenCL code at \num{4} cores.
